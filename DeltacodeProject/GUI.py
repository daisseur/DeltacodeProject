...
class App(CTk):
    def __init__(self):
        ...
        ico = ImageTk.PhotoImage(Image.open(f"..{sep}Deltacode.ico"))
        ...
        self.options.create_button("Mesures", command=self.print_size, pady=140)
        self.options.options_pack()

        self.read_setup()

        self.create_label_entry('', trace=self.var_password)
        self.create_label_entry('', trace=self.var_txt)
        self.create_label_entry('', trace=self.var_shift)
